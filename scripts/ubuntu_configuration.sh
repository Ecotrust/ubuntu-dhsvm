...
if [ $host == "ubuntu-bionic" ]; then
  ...
  cmake $options \
      # -D GA_TEST_RUNS:BOOL=OFF \
      # -D GA_DIR:PATH="/usr/local/apps/ucsrb2/ga" \
      -D GA_DIR:PATH="/usr/local/apps/ucsrb2/GridPACK" \
      -D DHSVM_SNOW_ONLY:BOOL=OFF \
      -D DHSVM_USE_X11:BOOL=OFF \
      -D DHSVM_USE_NETCDF:BOOL=OFF \
      -D DHSVM_BUILD_TESTS:BOOL=OFF \
      -D PETSC_DIR:STRING="/usr/lib/petscdir/3.7.7" \
